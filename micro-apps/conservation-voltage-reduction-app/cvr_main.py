...
class ConservationVoltageReductionController(object):
    ...
    def __init__(self,
        ...
        if not isinstance(simulation, Simulation) and simulation is not None:
            raise TypeError(
                f'The simulation arg must be a Simulation type or {None}!')
        self.id = uuid4()
        ...
        self.gad_obj.subscribe(measurements_topic,
                               self.on_measurement_callback)
        ...
        self.controllable_capacitors = self.graph_model.graph.get(
            ...
            for transformerTank in powerTransformer.TransformerTanks:
                for transformerEnd in transformerTank.TransformerTankEnds:
                    if transformerEnd.RatioTapChanger is not None and transformerEnd.Terminal.sequenceNumber == 1:
                        ...
                        self.controllable_regulators[mRID][
                            'name'] = f'reg_{powerTransformer.name}'
                        self.controllable_regulators[mRID][
                            f'ratioTapChanger'] = transformerEnd.RatioTapChanger
        ...
        for meas in measurements.values():
            if meas.measurementType == 'PNV':  #it's a voltage measurement. store it.
                mrid = meas.mRID
                self.pnv_measurements[mrid] = {
                    'measurement_object': meas,
                    'measurement_value': None
                }
            elif meas.measurementType == 'VA':  #it's a power measurement.
                if isinstance(
                        meas.PowerSystemResource,
                    ...
                    if mrid not in self.va_measurements.keys():
                        self.va_measurements[mrid] = {
                            'measurement_objects': {},
                            'measurement_values': {}
                        }
                    self.va_measurements[mrid]['measurement_objects'][
                        meas.mRID] = meas
                    self.va_measurements[mrid]['measurement_values'][
                        meas.mRID] = None
            elif meas.measurementType == 'Pos':
                if isinstance(meas.PowerSystemResource,
                              (cim.Switch, cim.PowerTransformer,
                               cim.LinearShuntCompensator)):
                    mrid = meas.PowerSystemResource.mRID
                    if mrid not in self.pos_measurements.keys():
                        self.pos_measurements[mrid] = {
                            'measurement_objects': {},
                            'measurement_values': {}
                        }
                    self.pos_measurements[mrid]['measurement_objects'][
                        meas.mRID] = meas
                    self.pos_measurements[mrid]['measurement_values'][
                        meas.mRID] = None
    ...
    def on_measurement(self, sim: Simulation, timestamp: str,
        ...
        for psr_mrid in self.va_measurements.keys():
            for mrid in self.va_measurements[psr_mrid][
                    'measurement_values'].keys():
                meas = measurements.get(mrid)
                if meas is not None:
                    self.va_measurements[psr_mrid]['measurement_values'][
                        mrid] = meas
        for psr_mrid in self.pos_measurements.keys():
            for mrid in self.pos_measurements[psr_mrid][
                    'measurement_values'].keys():
                meas = measurements.get(mrid)
                if meas is not None:
                    self.pos_measurements[psr_mrid]['measurement_values'][
                        mrid] = meas
    ...
    def calculate_per_unit_voltage(self):
        for mrid in self.pnv_measurements.keys():
            ...
            if isinstance(meas_term, cim.Terminal):
                if isinstance(meas_term.ConductingEquipment,
    ...
    def create_opendss_context(self):
        ...
        base_dss_dict = json.loads(base_dss_response.get('message', ''),
                                   strict=False)
        base_dss_str = base_dss_dict.get('data', '')
        fileDir = Path(
            __file__
        ).parent / 'cvr_app_instances' / f'{self.id}' / 'master.dss'
    ...
    def update_opendss_with_measurements(self):
        for psr_mrid in self.va_measurements.keys():
            ...
            val_is_valid = True
            for meas_mrid in self.va_measurements[psr_mrid][
                    'measurement_values'].keys():
                meas_val = self.va_measurements[psr_mrid][
                    'measurement_values'][meas_mrid]
                if meas_val is not None:
                    va_val += complex(
                        meas_val.get('magnitude') *
                        math.cos(math.radians(meas_val.get('angle'))),
                        meas_val.get('magnitude') *
                        math.sin(math.radians(meas_val.get('angle')))) / 1000.0
                else:
                    val_is_valid = False
                    break
            if val_is_valid:
                meas_obj = self.va_measurements[psr_mrid][
                    'measurement_objects'][meas_mrid].PowerSystemResource
                name = meas_obj.name
                if isinstance(meas_obj, cim.EnergyConsumer):
                    ...
                    if isinstance(meas_obj.PowerElectronicsUnit,
                                  cim.PhotovoltaicUnit):
                        self.dssContext.Command(
                            f'PVSystem.{name}.pmpp={va_val.real}')
                        self.dssContext.Command(
                            f'PVSystem.{name}.kvar={va_val.imag}')
                    elif isinstance(meas_obj.PowerElectronicsUnit,
                                    cim.BatteryUnit):
                        self.dssContext.Command(
                            f'Storage.{name}.kw={va_val.real}')
                        self.dssContext.Command(
                            f'Storage.{name}.kvar={va_val.imag}')
                    #TODO: update wind turbines in opendss
                #TODO: update other generator/motor type object in opendss
        for psr_mrid in self.pos_measurements.keys():
            num_of_meas = len(
                self.pos_measurements[psr_mrid]['measurement_objects'].keys())
            pos_val = [0] * num_of_meas
            val_is_valid = True
            key_list = list(
                self.pos_measurements[psr_mrid]['measurement_values'].keys())
            for meas_mrid in key_list:
                meas_val = self.pos_measurements[psr_mrid][
                    'measurement_values'][meas_mrid]
                if meas_val is not None:
                    ...
                    break
            if val_is_valid:
                meas_obj = self.pos_measurements[psr_mrid][
                    'measurement_objects'][meas_mrid].PowerSystemResource
                name = meas_obj.name
                if isinstance(meas_obj, cim.Switch):
                    ...
                    self.dssContext.Command(
                        f'Capacitor.{name}.states={pos_val}')
                if isinstance(meas_obj, cim.PowerTransformer):
                    ...
                    self.dssContext.Command(
                        f'Transformer.{name}.Taps=[1.0, {tapStep}]')
...
def buildGraphModel(mrid: str) -> FeederModel:
    if not isinstance(mrid, str):
        raise TypeError(
            f'The mrid passed to the convervation voltage reduction application must be a string.'
        )
    if mrid not in CIM_GRAPH_MODELS.keys():
        feeder_container = cim.Feeder(mRID=mrid)
        graph_model = FeederModel(connection=DB_CONNECTION,
                                  container=feeder_container,
                                  distributed=False)
...
def createSimulation(gad_obj: GridAPPSD, model_info: Dict[str,
                                                          Any]) -> Simulation:
    if not isinstance(gad_obj, GridAPPSD):
        raise TypeError(f'gad_obj must be an instance of GridAPPSD!')
    ...
    if line_name is None:
        raise ValueError(
            f'Bad model info dictionary. The dictionary is missing key modelId.'
        )
    if subregion_name is None:
        raise ValueError(
            f'Bad model info dictionary. The dictionary is missing key subRegionId.'
        )
    if region_name is None:
        raise ValueError(
            f'Bad model info dictionary. The dictionary is missing key regionId.'
        )
    if sim_name is None:
        raise ValueError(
            f'Bad model info dictionary. The dictionary is missing key modelName.'
        )
    ...
    sim_config = SimulationConfig(power_system_config=psc,
                                  simulation_config=sim_args)
...
def createGadObject() -> GridAPPSD:
    ...
    if gad_password is None:
        os.putenv('GRIDAPPSD_APPLICATION_ID',
...
def main(control_enabled: bool, start_simulations: bool, model_id: str = None):
    ...
    if not isinstance(model_id, str) and model_id is not None:
        raise TypeError(
            'The model id passed to the convervation voltage reduction application must be a string type '
            f'or {None}.')
    global cim, DB_CONNECTION
    cim_profile = 'rc4_2021'
    iec61970_301 = 7
    cim = importlib.import_module(f'cimgraph.data_profile.{cim_profile}')
    params = ConnectionParameters(cim_profile=cim_profile,
    ...
    app_instances = {
        'field_instances': {},
        'external_simulation_instances': {},
        'local_simulation_instances': {}
    }
    ...
    if model_id is not None:
        ...
        if not model_is_valid:
            raise ValueError(
                f'The model id provided does not exist in the GridAPPS-D plaform.'
            )
        else:
            models = [m]
    if start_simulations:
        for m in models:
            local_simulations[m.get('modelId',
                                    '')] = createSimulation(gad_object, m)
    else:
    ...
    for sim_id, m_id in platform_simulations.items():
        app_instances['external_simulation_instances'][
            sim_id] = ConservationVoltageReductionController(gad_object,
                                                             m_id,
                                                             sim_id=sim_id)
    ...
    if app_instances_exist:
        gad_log.info(
            'ConservationVoltageReductionApplication successfully started.')
        ...
        try:
            ...
            if not app_instances_exist:
                ...
                if application_downtime > 3600:
                    gad_log.info(
                        'There have been no running instances ConservationVoltageReductionController '
                        'instances for an hour. Shutting down the application.'
                    )
                    gad_object.set_application_status(
                        ProcessStatusEnum.STOPPING)
                    gad_object.disconnect()
        except KeyboardInterrupt:
            gad_log.info(
                'Manually exiting ConservationVoltageReductionApplication')
...
if __name__ == '__main__':
    ...
    parser.add_argument('model_id',
                        nargs='?',
                        default=None,
                        help='The mrid of the cim model to perform cvr on.')
